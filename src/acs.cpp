...
struct node nodeProb[nodeNum];    //可到达结点组
double lineNodeProb[nodeNum];     //线性化 可到达结点的选择概率
...
{
	int c = 0;//记录蚂蚁可行进的结点个数


	//1、计算可行进的各结点 选择概率
	for (int m = 0; m < nodeNum; m++)
	{
		//若结点（i,j）之间有路且j不在蚂蚁k的禁忌表中，则计算概率
